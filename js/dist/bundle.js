...
function calc() {
  // Calc
  var totalValue = document.querySelector('.calculating__result span');
  ...
  function calcTotal() {
    ...
    if (sex === 'female') {
      totalValue.textContent = Math.round((447.6 + 9.2 * weight + 3.1 * height - 4.3 * age) * ratio);
    } else {
      totalValue.textContent = Math.round((88.36 + 13.4 * weight + 4.8 * height - 5.7 * age) * ratio);
    }
  }
...
function cards() {
    ...
    _createClass(MenuCard, [{
      ...
      value: function render() {
        ...
        if (this.classes.length === 0) {
          // if [classes] is empty  
          this.classes = 'menu__item'; // fill the empty [classes] with default class

          element.classList.add(this.classes); // add default class for elem
        } else {
          this.classes.forEach(function (className) {
            element.classList.add(className); // add every new class for elem
          });
        }
  ...
  /* Разовое динамическое создание элл-мента 
...
function forms(formSelector, modalTimerId) {
  ...
  forms.forEach(function (item) {
    // initialize function for every form
    bindPostData(item);
  });
  var message = {
    loading: 'img/form/spinner.svg',
    success: "Success! You'll be noticed",
  ...
  function bindPostData(form) {
    ...
    form.addEventListener('submit', function (event) {
      ...
      form.insertAdjacentElement('afterend', statusMessage); // insert spinner after form 
      // Spinner

      var formData = new FormData(form); // get inputs value from forms
      // Convert to JSON

      var json = JSON.stringify(Object.fromEntries(formData.entries())); // переведет в массив
      // превратить в JSON      // fromEntriest вернет объект

      var obj = {
        a: 23,
        b: 50
      };
      console.log(Object.entries(obj)); // Convert to JSON
  ...
  function showThanksModal(message) {
    ...
    prevModalDialog.classList.add('hide'); // hide previous modal dialog

    Object(_modal__WEBPACK_IMPORTED_MODULE_10__["openModal"])('.modal', modalTimerId); // create NEW modal dialog
...
function openModal(modalSelector, modalTimerId) {
  ...
  document.body.style.overflow = 'hidden'; // no scroll while open
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  modalBtn.forEach(function (btn) {
    // add event click for every data-modal btn
  ...
  function showModalByScroll() {
    if (window.pageYOffset + document.documentElement.clientHeight == document.documentElement.scrollHeight) {
      openModal(modalSelector, modalTimerId); // don't show modal again by scroll

      window.removeEventListener('scroll', showModalByScroll);
    }
  }

  window.addEventListener('scroll', showModalByScroll); // when user reach bottom of the page 1st time
}
...
function slider(_ref) {
  ...
  var initializeSlider = function initializeSlider() {
    currentSlide.textContent = Object(_timer__WEBPACK_IMPORTED_MODULE_4__["getZero"])(sliderIndex);
    totalSlides.textContent = Object(_timer__WEBPACK_IMPORTED_MODULE_4__["getZero"])(slides.length);
  };
  ...
  sliderWindow.style.overflow = 'hidden'; // cant see slides behind the wall
  ...
  sliderWindow.append(indicators); // Styling
  // Create dots == pics sum
  ...
  dots.forEach(function (dot) {
    dot.addEventListener('click', function (e) {
      var slideTo = e.target.getAttribute('data-dot');
      sliderIndex = slideTo;
...
function timer(timerSelector, deadLine) {
  ...
  function getTimeRemaining() {
    var endtime = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : deadLine;
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.tabheader__items', '.tabheader__item', '.tabcontent', 'tabheader__item_active');
  Object(_modules_modal__WEBPACK_IMPORTED_MODULE_3__["default"])('[data-modalBtn]', '.modal', modalTimerId);
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_4__["default"])('.timer', '2021-01-01 00:00');
